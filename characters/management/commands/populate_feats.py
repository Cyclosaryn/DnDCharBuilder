...
class Command(BaseCommand):
    help = 'Populate database with D&D 5e feats from Basic Rules and common feats'
    ...
    def handle(self, *args, **kwargs):
        feats_data = [
            ...
            },
            {
                'name': 'Skulker',
                'prerequisite': 'Dexterity 13 or higher',
                'description': 'You are an expert at slinking through shadows. You gain the following benefits:\n• You can try to hide when you are lightly obscured from the creature from which you are hiding.\n• When you are hidden from a creature and miss it with a ranged weapon attack, making the attack doesn\'t reveal your position.\n• Dim light doesn\'t impose disadvantage on your Wisdom (Perception) checks relying on sight.'
            },
            {
                'name': 'Spell Sniper',
                'prerequisite': 'The ability to cast at least one spell',
        ...
        for feat_data in feats_data:
            feat, created = Feat.objects.get_or_create(
                name=feat_data['name'],
                defaults={
                    'prerequisite': feat_data['prerequisite'],
        ...
        self.stdout.write(self.style.SUCCESS(f'\nSuccessfully populated {Feat.objects.count()} feats!'))
